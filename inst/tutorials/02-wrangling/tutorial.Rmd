...
<!-- Even if you need a special object called lexus_1998, just create once. -->
...
`library(babynames)`
`library(ggthemes)`
If you have not installed these packages, you will like encounter issues when attempting the tutorial, so make sure to do so. If you don't remember how to do so, reference the `install.packages()` function introduced earlier in the course. 
...
### Exercise 1
Let's use a dataset in the `fueleconomy` package to better understand factors. First, `glimpse()` the `vehicles` dataset. 
...
### Exercise 4
Create a `mean_cty` variable using the `mutate()` and `mean()` functions for the `vehicles_fct` dataframe. Reassign this mutated dataframe as `vehicles_fct`.
```{r factors-4-setup}
vehicles_fct <- vehicles %>%
  mutate(class = as.factor(class)) %>%
  group_by(class)
...
# Consider using the c() function to create the individual vectors for a, b, and c
...
### Exercise 3
Use the `make_datetime()` function to create a date-time for Y2K. 
...
```{r q2, echo = FALSE}
question("Do the two data datasets above contain the variables **country**, **year**, **cases**, and **population**?",
...
The `pollution` dataset below displays the amount of small and large particulate in the air of three cities. It illustrates another common type of untidy data. **Narrow data** uses a literal key column and a literal value column to store multiple variables. Can you tell here which is which? Run the code chunk below. 
...
Call `babynames` first to see the dataset.
...
```{r nes4-hint, eval=FALSE}
# Remember to use the %in% operator alongside the c() function.
```

###

After filtering a factor, always remember that the *levels* of the factor have not been removed. To fix this, add a call within the pipe that drops (i.e. permanently removes) all deleted levels.
...
nes %>%
  ...
  droplevels() %>% 
  mutate(pos_ideology = 
           as.factor(if_else(condition = ideology > 0,
...
```{r nes7-hint-1, eval=FALSE}
# Remember to use as.factor() before if_else().
```

```{r nes7-hint-2, eval=FALSE}
...
Great job! Now, to calculate turnout, simply `mutate()` another variable, `turnout`, equal to the number of people who voted (the number of `Yes`) divided by the total number of people. To make sure you are caught up, we provide the pipe as it should appear by this stage. You just need to add a line to create `turnout`.
...
To finish your plot, use `labs()` to give the graph a title and subtitle of your choosing. Here is our answer. It is OK if your answer looks somewhat different from ours.
...
nes %>%
  ...
  ggplot(mapping = aes(x = year,
    ...
    labs(title = "Opinions on Presidents Over Time",
         subtitle = "People with opinions are more educated")
```
...
We will be focusing on the data set `airline_safety` from the `fivethirtyeight` package. Start by taking a `glimpse` of `airline_safety`:
...
```{r as-8, exercise=TRUE, exercise.lines = 10}
...
```{r as-10, exercise=TRUE, exercise.lines=15}
...
Great! Now add some labels to finish our plot off! Here is our version. As always, it is OK if yours looks different.

```{r as-11-answer, echo=FALSE}
 airline_safety %>%     
...
```{r as-11, exercise=TRUE, exercise.lines=20}
