...
nes_subset <- nes %>%
  ...
  pivot_wider(names_from = voted, values_from = count) %>%
  mutate(turnout = Yes / (Yes + No)) %>%
nes_plot <- ggplot(data = nes_subset, mapping = aes(x = year, y = turnout, color = education, linetype = education)) +
    geom_line()
```
...
nes_plot_3 <- ggplot(data = nes_subset, mapping = aes(x = year, y = turnout, color = fct_reorder2(education, year, turnout, .fun = ...), linetype = fct_reorder2(education, year, turnout, .fun = ...))) +
    geom_line() +
    facet_wrap(~ opinion)
```
...
nes_plot_3 <- ggplot(mapping = aes(x = year, y = turnout, color = fct_reorder2(education, year, turnout, .fun = first2), linetype = fct_reorder2(education, year, turnout, .fun = first2))) +
    geom_line() +
    facet_wrap(~ opinion)
```
...
## Wrangling: daily_show_guests

Let's do some data wrangling with the `daily_show_guests` tibble from the `fivethirtyeight` tibble. Run `library(fivethirtyeight)` to load up the `fivethirtyeight` package. Then, run `daily_show_guests` to get a run-down on the data in the tibble. Unlike many of the other tibbles we've worked with in this chapter, this one only contains 5 columns, so we don't need to use `glimpse()` or `skim()` to get a grasp of all the data.
...
Note that in this tutorial set, we will be working within a single pipe (`%>%`) chain. Feel free to copy and paste from question to question.

```{r dw-5-1, exercise=TRUE, exercise.lines = 5}

```

###

Cool. Although we don't have a ton of columns, that doesn't we're done with them. If you notice, some of our columns have needlessly long and confusing names. 
We can tackle this with `rename()`. Let's rename our columns such that `occupation` = `google_knowledge_occupation`, and `name` = `raw_guest_list`. Although this won't change anything about our data, it will make the tibble easier for us to work with.

```{r dw-5-2, exercise=TRUE, exercise.lines = 5}

```

###

Great! Now, let's get wrangling. Let's `filter()` our data, so that `occupation` == "actor" (we use the double = instead of single here, because we're using = as a comparative operator instead of an assignment operator). As you probably guessed, this will whittle down our datapoints to only actors.

```{r dw-5-3, exercise=TRUE, exercise.lines = 5}

```

###

Nicely done. Now, let's try using regex to filter down our data even further. In order to use string functions, which we will need to do to use regex, we will first need to turn our tibble into an atomic vector. But how can we do that? Well, let's begin by using `select()` to pick only the `name` column.

```{r dw-5-4, exercise=TRUE, exercise.lines = 5}

```

###

Good. Now, we can use the `deframe()` function to turn our one-row tibble into an atomic vector.

```{r dw-5-5, exercise=TRUE, exercise.lines = 5}

```

###

Wonderful. Now, let's get regexing! 

Use `str_subset(pattern = "")` to sort for every actor's name that begins with the letter `C`.

```{r dw-5-6-hint, eval=FALSE}
The `^` operator stands for "begins with" in regex. 
```

```{r dw-5-6, exercise=TRUE, exercise.lines = 5}

```

###

Nice work. But as I'm sure you've noticed, we have quite a few duplicate names. Colin Firth alone shows up 7 times!

We can remove these duplicates using the `unqiue()` function. Try it!

```{r dw-5-7, exercise=TRUE, exercise.lines = 5}
...
```{r dw-5-8, exercise=TRUE, exercise.lines = 5}

```

###

Good. Now, let's use `str_replace_all()` to change every capital letter in each name to the letter "P". This doesn't really accomplish anything productive, but I think it's kind of funny.

```{r dw-5-9-hint, eval=FALSE}
In regex, "[A-Z]" stands for every capital letter. ([a-z] and [0-9] also work in a similar way, in case you were wondering.)
```

```{r dw-5-9, exercise=TRUE, exercise.lines = 5}

```

###

Very good. Now, let's use `str_remove()` to get remove the first "i" in each name. 

Note that if we wanted to remove every "i", we could use `str_remove_all()`

```{r dw-5-10, exercise=TRUE, exercise.lines = 5}

```

###

Congratulations, you're finished. If you did everything right, your character vector should look a little something like this. We've definitely completed our goal of making fake names similar to popular celebrities. 

```{r}
daily_show_guests %>% 
  filter(google_knowledge_occupation == "actor") %>% 
  select(raw_guest_list) %>% 
  rename(name = raw_guest_list) %>% 
  deframe() %>% 
  str_subset(pattern = "^C") %>% 
  unique() %>% 
  str_subset(pattern = "r.*n") %>% 
  str_replace_all("P", pattern = "[A-Z]") %>% 
  str_remove(pattern = "i")
```

## Wrangling: fandango

Let's do some data wrangling with the `fandango` tibble from the `fivethirtyeight` package. Begin by running `libarary(fivethirtyeight)` to load up the package, and the run `glimpse()` on `fandango`, so we can get a grasp on all of its data.

```{r dw-6-1, exercise=TRUE, exercise.lines = 5}

```

###

Nice. Now, let's get rid of our `glimpse()` function - from here on out, we will be working within a single pipe (`%>%`) chain. Feel free to copy and paste from question to question.

Let's begin by using `select()` to pick out the `film`, `fandango_stars`, `metacritic_norm_round`, `imdb_norm_round`, and `rt_norm_round` columns.

```{r dw-6-2, exercise=TRUE, exercise.lines = 5}

```

###

Cool. Now let's use `rename()` to tidy up our variable names. Make `fandango` = `fandango_stars`, `metacritic` = `metacritic_norm_round`, `imdb` = `imdb_norm_round` and `rotten_tomatoes` = `rt_norm_round`.

```{r dw-6-3, exercise=TRUE, exercise.lines = 5}

```

###

Good work. Now, let's use `mutate()` to make a new column, `avg`, that's equal to the average score across all four of our columns. Because we're not technically working with a vector, we can't use `summarise(mean())` without some major modifications to our tibble. We can, however, manually calculate an average within a `mutate()` function, by adding all four values together and then dividing the total by 4. 

```{r dw-6-4, exercise=TRUE, exercise.lines = 5}

```

###

Great. But let's say we want to make `avg` rounded to the nearest 0.5, like every other column. 
...
Change `avg` to be equal to `round()` of `avg * 2`, to `0` decimal places and then dividing the result by 2. (Setting the second argument of `round` to 0 rounds to the nearest integer, if you wish to round to a non-decimal power of 10, you can do so by setting your second argument to a negative number).

```{r dw-6-5-hint, eval=FALSE}
mutate(avg = round(..., ...)/2)
```

```{r dw-6-5, exercise=TRUE, exercise.lines = 5}

```

###

Nice job. Now that we have all of our columns, we can make our data tidy using the `pivot_longer()` function. We're going to be using every column except for `film`, putting our variable names into a new column called `critic_website`, and our values into a new column called `score`.

```{r dw-6-6-hint, eval=FALSE}
... %>% pivot_longer(cols = -film, names_to = "...", values_to = "...")
```

```{r dw-6-6, exercise=TRUE, exercise.lines = 5}

```

###

Nice. Now, let's use our tidy data to make a boxplot with `ggplot()` `geom_boxplot()` Map x to `critic_website` and y to `score`.

```{r dw-6-8, exercise=TRUE, exercise.lines = 5}

```

###

Well done. Now, let's add a scatterplot layer on top of our boxplot with `geom_jitter()` (`geom_point()` would lead to a lot of overplotting here!). While we're at it, let's also use `theme_classic()` to change the theme of our plot.

```{r dw-6-9, exercise=TRUE, exercise.lines = 5}
...
```{r dw-6-10, exercise=TRUE, exercise.lines = 5}

```

###

Congrats! You finished the tutorial set. If you did everything right, your plot should look a little something like this.

```{r}
fandango %>% 
  select(film, fandango_stars, metacritic_norm_round, imdb_norm_round, rt_norm_round) %>% 
  rename(fandango = fandango_stars, metacritic = metacritic_norm_round, imdb = imdb_norm_round, rotten_tomatoes = rt_norm_round) %>% 
  mutate(avg = (fandango + metacritic + imdb + rotten_tomatoes) / 4) %>% 
  mutate(avg = round(avg * 2, 0)/2) %>% 
  pivot_longer(cols = -film, names_to = "critic_website", values_to = "score") %>% 
  ggplot(aes(x = critic_website, y = score)) +
  geom_boxplot() + 
  geom_jitter() + 
  theme_classic() + 
    labs(title = "Average Movie Scores from Different Critic Wesbites", 
       x = "Critic Website", 
       y = "Score (Out of 5)")
```

## Wrangling: foul_balls

Let's do some data wrangling with the `foul_balls` tibble from the `fivethirtyeight` package. Begin by running `library(fivethrityeight)` and `library(skimr)` to call the respective packages, then run `skim(library)` so we can get a good grasp on the tibble.

```{r dw-7-1, exercise=TRUE, exercise.lines = 5}

```

###

Our first goal with this exercise will be to see and plot the amount of ground balls from month to month.

Let's begin how we usually do. `select()` the `matchup`, `game_date`, `predicted_zone` and `type_of_hit` columns.

```{r dw-7-2, exercise=TRUE, exercise.lines = 5}

```

###

Very nice. Now, let's use `filter()` to select every row where `type_of_hit` == "Ground"

```{r dw-7-3, exercise=TRUE, exercise.lines = 5}

```

###

Good stuff. But let's say we didn't want to include any data from `predicted_zone` = 1. Well, we could use a filtering join, specifically, `anti_join()`.

Hit run code on the block of code below to create a new tibble, `x`, equal to all the rows of data with `predicted_zone = 1`.


```{r, include=FALSE}
x <- foul_balls %>% filter(predicted_zone == 1)
```

```{r dw-7-4, exercise=TRUE, exercise.lines = 5}
x <- foul_balls %>% filter(predicted_zone == 1)
```

###

Nice, now, we can pipe our previous block of code into `anti_join(x)`, to filter out all the values in `x` from our tibble. Try it!

```{r dw-7-5, exercise=TRUE, exercise.lines = 5}
x <- foul_balls %>% filter(predicted_zone == 1)
```

###

Good. Now, let's use `mutate()` and a lubridate function to make a new column, `date`, out of `game_date`. In this case, we will be using `ymd()`, because the dates are organized in the order `year, month, day`.

If you've been astute you may have noticed that we don't actually need to do this, because the `game_date` column is already a date/time, but it is good practice for if any time in your data science futures you will have to deal with character dates.

```{r dw-7-6, exercise=TRUE, exercise.lines = 5}

```

###

Cool. Now, we can use `mutate()` yet again, along with the `month()` function on `date` to make a new column, which we'll call `game_month`.

```{r dw-7-7, exercise=TRUE, exercise.lines = 5}

```

###

Nice work. Now, let's use `group_by()` to group our data by `game_month`.

```{r dw-7-8, exercise=TRUE, exercise.lines = 5}

```

###

Great, now use `summarise()` to make a new column, `count`, equal to the `n()`, or number of rows, for each value of `game_month` in our tibble. This function does not require an argument in `n()`.

```{r dw-7-9, exercise=TRUE, exercise.lines = 5}

```

###

Very good. Now, we're ready to plot. Use `ggplot()` and `geom_line()` to make a line plot with x = `game_month` and y = `count`.

```{r dw-7-10, exercise=TRUE, exercise.lines = 5}
